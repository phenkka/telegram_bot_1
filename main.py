    ...
    while True:
        ...
        for token in tokens:
            ...
            if not db.is_token_notified(token):
                ...
                message_smart = f"🔔 The token <a href='https://dexscreener.com/solana/{token}'>{token}</a> is being actively bought!\nHere's the list:\n\n"
                message_infl = f"🔔 The token <a href='https://dexscreener.com/solana/{token}'>{token}</a> is being actively bought!\nHere's the list:\n\n"
