...
class Database:
    ...
    def remove_expired_users(self):
        """Удаляет пользователей, чья оплата истекла более 120!!!!!!!!!!!!!!!!!!!!! дней назад"""
    ...
    def get_notify_infl(self, user_id):
        notify_status_query = "SELECT notify_infl FROM users WHERE user_id = %s"
        result_notify_status = self.execute_read_one_query(notify_status_query, params=(user_id,))
        return result_notify_status[0] if result_notify_status else False

    def get_notify_smart(self, user_id):
        notify_status_query = "SELECT notify_smart FROM users WHERE user_id = %s"
        result_notify_status = self.execute_read_one_query(notify_status_query, params=(user_id,))
        return result_notify_status[0] if result_notify_status else False
    ...
    def update_notify_infl_status(self, user_id, new_status):
        update_query = "UPDATE users SET notify_infl = %s WHERE user_id = %s"
        self.execute_write_query(update_query, params=(new_status, user_id))

    def update_notify_smart_status(self, user_id, new_status):
        update_query = "UPDATE users SET notify_smart = %s WHERE user_id = %s"
        self.execute_write_query(update_query, params=(new_status, user_id))

    def get_users_with_notifications(self):
        not_notify_status_query = "SELECT user_id FROM users WHERE notify_smart = TRUE OR notify_infl = TRUE"
