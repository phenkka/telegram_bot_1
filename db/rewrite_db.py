...
class Database:
    ...
    def update_payment_status(self, user_id, status):
        ...
        '''
        self.execute_write_query(update_query, params=(user_id, status, payment_date, status, payment_date))
    ...
    def is_payment_valid(self, user_id):
        ...
        if result_status_payment:
            payment_date = datetime.datetime.fromisoformat(result_status_payment[0])
    ...
    def is_token_notified(self, token):
        ...
        return self.execute_read_one_query(notified_query, params=(token,))
...
# Не изменял, тк у нас пнл и вр через коленку выводится и пока лучше так оставить
class ImportDB:
